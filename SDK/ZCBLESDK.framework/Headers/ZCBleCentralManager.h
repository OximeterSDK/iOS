...
#import "ZCODeviceModel.h"
#import "ZCBlePeripheralProtocol.h"

#define ZC(weakSelf)  __weak __typeof(&*self)weakSelf = self;
...
typedef NS_ENUM(NSInteger, ZCBleConnectState) {
    ...
    ZCBleConnectStateDiscoverCanSendCommand, //发现了我们自己的服务，只有这个被找到后才可以正常收发指令
};

typedef NS_ENUM(NSInteger, ZCCentralManagerState) {
    ZCCentralManagerStateUnknown = 0,  //系统蓝牙未知
    ZCCentralManagerStateResetting,    //没有使用
    ZCCentralManagerStateUnsupport,    //设备不支持
    ZCCentralManagerStateUnauthorized, //未授权
...
//扫描设备回掉
typedef void(^ZCBleScanBlock)(NSArray <ZCODeviceModel *>*peripheralModel);
...
//设备发现服务
typedef void(^ZCBleDiscoverServicesBlock)(ZCODeviceModel *peripheralModel);
//设备发现服务下的特征
...
@interface ZCBleCentralManager : NSObject<CBCentralManagerDelegate, CBPeripheralDelegate>
...
//已经连接设备的模型
@property (nonatomic, strong) ZCODeviceModel *peripheralModel;

//在非主动断开连接后,是否需要主动连接设备,默认为YES
@property (nonatomic, assign) BOOL autoConnect;

//设备是否连接，如果App中在不同界面需要监听蓝牙连接状态，可以用KVC监听这个属性，当然也可以在蓝牙状态block中增加通知
@property (nonatomic, assign) BOOL connect;
//当前连接状态
@property (nonatomic, assign) ZCBleConnectState connectState;
...
//连接设备
- (void)zc_connectPeripheralModel:(ZCODeviceModel *)model;

//断开连接，model可以传nil
- (void)zc_cancelPeripheralModel:(ZCODeviceModel *)model;
